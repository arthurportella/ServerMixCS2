...
document.addEventListener('click', async function(e) {
    ...
    e.target.classList.contains('counter-terror')) {
        ...
        try {
            ...
            const fetchUpdate = await fetch(`${Prefix}update`, {
                method: 'post',
                body: formdata
            });
            const response = await fetchUpdate.text();

            ToggleLoading(true);

            if(response.length != 0) {
                AddMessage(`Error: ${response}`, 'fail', 10000);
                return;
            }
...
async function ApplySticker(imgsrc = '', place = 0) {
    if(!imgsrc || imgsrc == '' || !model) {return false;}

    if(stickerspreview[place]) {
        let decal = stickerspreview[place];
        decal.material.map.dispose();

        let stickerimg = await new THREE.TextureLoader().load(imgsrc);
        stickerimg.encoding = THREE.sRGBEncoding;
        stickerimg.colorSpace = THREE.SRGBColorSpace;

        decal.material.map = stickerimg;
    ...
    const size = await box.getSize(new THREE.Vector3());

    const stickerimage = await new THREE.TextureLoader().load(imgsrc);
    stickerimage.encoding = THREE.sRGBEncoding;
    stickerimage.colorSpace = THREE.SRGBColorSpace;

    const stickermats = await new THREE.MeshBasicMaterial({
        map: stickerimage,
        transparent: true,
        depthTest: true,
        depthWrite: false,
    ...
    let pos = positions[place] || positions[0];

    let decal = await new THREE.Mesh(
        new DecalGeometry(
            mesh,
            new THREE.Vector3((pos[0]/100)*(size.x/2), (pos[1]/100)*(size.y/2),-size.z),
            new THREE.Euler(0,0,0),
            // new THREE.Vector3(-2.5, 2.2,size.z*1.7)
            new THREE.Vector3(-2.8, 2.2,size.z*1.7)
        ),
        stickermats
    );
